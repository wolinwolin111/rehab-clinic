...
_ALLOWED_COLUMNS = {
    ...
    'consumption': {'patient_id', 'month', 'day', 'project', 'amount', 'time_slot', 'sync_to_apt'},
...
def create_appointment():
    ...
    if data.get('sync_to_con'):
        conn.execute(
            ...
            (patient_id, month, day, project, amount, time_slot, 1)
        )
...
def update_appointment(aid):
    ...
    updates = []
    params = []
    fields = ['patient_id', 'year', 'month', 'day', 'time_slot', 'project', 'amount', 'remark']
    ...
    for link in linked_cons:
        ...
        cur = conn.execute(
            "UPDATE consumption SET month=?, day=?, project=?, amount=?, time_slot=?, sync_to_apt=1 WHERE id=?",
            (apt['month'], apt['day'], apt['project'], apt['amount'], apt['time_slot'], cid)
        )
        if cur.rowcount == 0:
            # 如果 ID 不存在，尝试通过字段匹配
            cur = conn.execute(
                "UPDATE consumption SET month=?, day=?, project=?, amount=?, time_slot=?, sync_to_apt=1 WHERE patient_id=? AND month=? AND day=? AND project=? AND amount=? AND time_slot=? AND sync_to_apt=1",
                (apt['month'], apt['day'], apt['project'], apt['amount'], apt['time_slot'],
                 old['patient_id'], old['month'], old['day'], old['project'], old['amount'], old['time_slot'])
            )
    conn.commit()
    conn.close()
    return jsonify(apt)
...
def create_consumption():
    ...
    conn.execute(
        "INSERT INTO consumption (patient_id, month, day, project, amount, time_slot, sync_to_apt) VALUES (?,?,?,?,?,?,?)",
        (patient_id, month, day, project, amount, time_slot, sync_to_apt)
    )
    conn.commit()
...
def update_consumption(cid):
    ...
    updates = []
    params = []
    fields = ['patient_id', 'month', 'day', 'project', 'amount', 'time_slot', 'sync_to_apt']
        ...
        try:
            ...
            if act == 'add':
                ...
                if ttype == 'appointment':
                    conn.execute(
                        ...
                        (data.get('patient_id'), data.get('year'), data.get('month'), data.get('day'),
                         data.get('time_slot',''), data.get('project',''), data.get('amount',0), data.get('remark',''))
                    )
                ...
                if ttype == 'appointment':
                    ...
                    conn.execute(
                        "UPDATE appointments SET patient_id=?, month=?, day=?, time_slot=?, project=?, amount=?, remark=? WHERE id=?",
                        (old.get('patient_id'), old.get('month'), old.get('day'),
                         old.get('time_slot',''), old.get('project',''), old.get('amount',0), old_remark, tid)
                    )
                    # 恢复关联耗卡
                    m = re.search(r'#(\d+)$', old_remark) if old_remark else None
                    if m:
                        conn.execute(
                            "UPDATE consumption SET patient_id=?, month=?, day=?, project=?, amount=?, time_slot=? WHERE id=?",
                            (old.get('patient_id'), old.get('month'), old.get('day'),
                             old.get('project',''), old.get('amount',0), old.get('time_slot',''), int(m.group(1)))
                        )
                    # 同步Excel
